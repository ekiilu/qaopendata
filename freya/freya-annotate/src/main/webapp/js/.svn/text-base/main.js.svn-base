	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status != 404) {
				...
				if (data.textResponse == null || data.textResponse.clarifications == null) {
...
//							right-container
				...
				//alert("now rendering dialog");;
					...
					if (data.textResponse.clarifications.length > 0) {
						//alert("data in render table: "+data);
						renderClarificationsTable(tBody, data);
					} else {
