...
public class Mapper {
    ...
    @Autowired RenameAnnotations renameAnnotations;
    @Autowired StanfordExecutor stanfordExecutor;
    @Autowired OCCreator ocCreator;
     ...
     * @return
     */
    public List<Annotation> fromTokensToLuceneLookupAnnotations(List<HasWord> tokens, Tree root,
                    boolean preferLonger) {
        List<Annotation> lookups = new ArrayList<Annotation>();
        logger.debug("There are this many tokens in the query:" + tokens.size());
        for (int start = 0; start < tokens.size(); start++) {
            // Tree thisTree =
            // locateTheTreeWithThisWord(root, start, tokens.get(start).word());
            ...
            for (int end = start + 1; end <= tokens.size(); end++) {
                ...
                boolean foundTwoWordsToIgnore =
                                checkWhetherThisIsATwoWordToIgnore(root, start, end);
                if (start == end - 2 && foundTwoWordsToIgnore) {
                    logger.debug("NOT sending this to lucene (found in ignore list):" + start
                                    + " end:" + end);
                    break;
                } else if (start == end - 1 && foundWordToIgnore) {
                    logger.debug("NOT sending this to lucene (found in ignore list):"
                                    + thisTree.toString());
                    ...
                    logger.debug("NOT sending this to lucene (probably phrase):"
                                    + thisTree.toString());
                } else {
                    ...
                    if (phraseRequiresSpecialTreatment(trees, root)) {
                        ...
                        for (Tree aTree : trees) {
                            if (aTree.label().value().contains("JJ")
                                            && !aTree.parent(root).label().value().startsWith("WH"))
                                newTrees.add(aTree);
                            if (aTree.label().value().contains("NN")) newTrees.add(aTree);
                        }
                    ...
                    if (tmplookups.size() > 0)
                        logger.info("Found " + tmplookups.size()
                                        + " entries in the lucene index for " + ann.getText());
                }
            }
        }
        ...
        // rename here
        logger.debug("Before renaming:" + lookups.size());
        int before = lookups.size();
        long start = System.currentTimeMillis();
        List<Annotation> toReturn = renameAnnotations.rename(lookups);
        long end = System.currentTimeMillis();
        long runtime = end - start;
        logger.info("Renaming for " + runtime + " ms. There are " + toReturn.size()
                        + " annotations.");
    ...
    public List<Annotation> findLiteral(Annotation annotation) {
        List<Annotation> anns = luceneAnnotator.searchIndex(annotation, false);
