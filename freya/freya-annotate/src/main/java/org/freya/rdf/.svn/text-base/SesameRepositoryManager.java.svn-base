...
public class SesameRepositoryManager implements RepositoryManager {
    ...
    public void loadResources(org.springframework.core.io.Resource[] resources) {
        ...
        for (org.springframework.core.io.Resource resource : resources) {
            ...
            if (!skip) {
                ...
                //log.info("Started loading {} ({}/{})", uri, currentFileNumber, numberOfFilesToLoad);
                try {
                    connection.add(resource.getInputStream(), "http://example.org#", format);
                } catch (Exception /* RDFParseException | RepositoryException | IOException */e) {
    ...
    void initLocalRepository() {
        ...
        try {
            ResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
            org.springframework.core.io.Resource[] resources = resolver.getResources(config);
            configFileInputStream = resources[0].getInputStream();

            repositoryManager = new LocalRepositoryManager(new File(localRepositoryManagerDir));

            repositoryManager.initialize();
            log.debug("Local repository initialised, ontologies not yet added");
        } catch (Exception e) {
            log.error("error loading local repository, configFile:{}", config, e);
            throw new RuntimeException(e);
        }
        ...
        try {
            ...
            Iterator<Statement> iter =
                            graph.match(null, RDF.TYPE, new URIImpl(
...
//                            repositoryId, repositoryURL, e);
        ...
        log.info("afterPropertiesSet() - Successfully initialised Sesame Repository using impl: {}", repository.getClass()
                        .getName());
    }
    /**
     * This method is called to reload {@link #preloadInputStream} to repository.
     * 
     * @param sourceUri A string of source file URI.
    ...
    public void loadByInputStream(String sourceUri, InputStream preloadInputStream) throws RepositoryException {
        ...
        try {
           ...
           // this.conn.add(preloadInputStream, "http://example.org#", format, resource);
           // this.conn.commit();
        } catch (IOException e) {
