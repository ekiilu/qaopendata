   ...
   static InputStream synMapStream;
    ...
    public static void makeIndex(IndexWriter writer, String baseQuery,
        ...
        try {
            ...
            for (String filterUri : filterUris) {
                ...
                logger.info("Processing query with filterURI(" + countFilter
                ...
                logger.info("Finished SPARQL query evaluation for:"
                                + (endQ - startQ) + "ms");
                long start = System.currentTimeMillis();
                indexTriples(writer, result, stem, filterUri, camelCase);
                long end = System.currentTimeMillis();
                logger.info("Finished reading from SPARQL and adding to lucene for:"
                                                + (end - start) + "ms");

                if (countFilter % 10000 == 0)
                    logger.info(" Processing " + countFilter + " filter URI");
                countFilter++;
                logger.info("Finished " + filterUri + " for:"
    ...
    static void indexTriples(IndexWriter writer,
        ...
        try {
            ...
            while (result.hasNext()) {
                ...
                long end = System.currentTimeMillis();
                if ((end - start) > 100)
                    System.out.println(rowCounter + "r for "
                ...
                if (contentWords.size() == 0) {
                    ...
                    logger.info("Adding " + instanceURI.getLocalName() + " because label is null");
                }
        ...
        try {
            ...
            while (result.hasNext()) {
                ...
                if (!aClass.stringValue().equals(aSubClass.stringValue())) {
                    ...
                    writer.addDocument(doc);
                    if (countDocuments % 10000 == 0)
                        System.out.print(" " + countDocuments + " docs ");
                    countDocuments++;
                }
            }
            result.close();
                ...
                if (domain != null) {
                    ...
                    Field classField =
                                    new Field(FreyaConstants.CLASS_FEATURE_LKB, domain
                                                    .stringValue(), Field.Store.YES, Field.Index.ANALYZED);
                    logger.info("Adding class:" + domain.stringValue());
                    Field predField =
                                    new Field(FreyaConstants.PROPERTY_FEATURE_LKB, domainUri,
                                                    Field.Store.YES, Field.Index.ANALYZED);
                    logger.info("Adding pred:" + domainUri);
                    doc.add(instField);
                    doc.add(classField);
                    doc.add(predField);
                    writer.addDocument(doc);
    ...
    String extendQuery(String baseQuery, String filterURI,
        ...
        return (baseQuery.substring(0, whereStart)).trim() + "\n"
                        + additionalStatement + baseQuery.substring(whereStart);
    }
